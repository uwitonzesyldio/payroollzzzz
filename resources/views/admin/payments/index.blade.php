<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.payments.create') }}" class="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-amber-500 to-orange-600 border border-transparent rounded-xl font-semibold text-xs text-white uppercase tracking-widest hover:from-amber-600 hover:to-orange-700 focus:outline-none focus:ring-2 focus:ring-amber-500 focus:ring-offset-2 transition-all duration-150 shadow-md hover:shadow-lg">
                    ...
                    <table class="min-w-full divide-y divide-gray-100">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-amber-50/40 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-semibold text-gray-900">{{ $payment->employee->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-emerald-600">${{ number_format($payment->amount, 2) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $payment->date->format('M d, Y') }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm">
                                    <div class="flex items-center space-x-2">
                                        <a href="{{ route('admin.payments.show', $payment) }}" class="inline-flex items-center px-3 py-1.5 bg-emerald-50 text-emerald-700 text-xs font-medium rounded-lg hover:bg-emerald-100 transition-colors">
                                        ...
                                        <a href="{{ route('admin.payments.edit', $payment) }}" class="inline-flex items-center px-3 py-1.5 bg-amber-50 text-amber-700 text-xs font-medium rounded-lg hover:bg-amber-100 transition-colors">
                                            <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr>
                                <td colspan="5" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        <p class="text-lg font-medium text-gray-600">No payments recorded yet</p>
                                    </div>
                                </td>
                            </tr>
                            @endforelse
